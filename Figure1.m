...
for i = 1:numEnsembles
    ...
    nexttile(i)
    importantNumbers = misfitPanel(ewre2n,data,...
        allModels,tileLetter,titles{i})
    tileLetter = char(tileLetter+1);
    nexttile(i+numEnsembles,[2 1])
    modelSpacePanel(binCenters,numElements,allModels,tileLetter);
    tileLetter = char(tileLetter+1);
    if i == 1
