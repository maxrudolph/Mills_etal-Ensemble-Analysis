function apparentResistivity = calculateRho1D(depths,rhos,lambda)
    %{
    depths (column vector; depths of layer interfaces), NaNs okay
    rhos (column vector; resistivities of each layer), NaNs okay 
    lambda is a meshgrid based on the x and abcissae; defined outside this 
    fxn which massively cut down on runtime. The lambda values are from the
    first equation, page 512, Guptasarma 1982. 
    Note this fxn relies on using an 11-point filter
    %}
...
%filter coefficients (CONSTANTS)11-point filter, Guptasarma 1982 table a-2
   filter = [
    ...
    %Now calculate T_1(lambda_k) for each k. Start with T_n = rho_n at the
