...
%Step 0: Define forward model
forwardModel = @(a,b,c) calculateRho1D(a,b,c);

%% Step 1: Load or generate synthetic data
ifLoadData = false;%input('Load data? true/false\n');

if ifLoadData
    % data = load(...)
    % ...Not setup yet...
else %Setup artificial model and measurement details. Schlumberger
    addpath(genpath('Step1DataGenerationScripts'));
    measure.subStructChoice = '3LayerA';
    %String corresponding to which subsurface structure to make
    %measurements on. See subStructGen for options or to make a new one.
    measure.minDist = 0.1; measure.maxDist = 1000; 
    % Smallest and largest electrode distances, in meters
    measure.numMeasurements = 21; %total # of measurements
    measure.noiseCoef = 0.11; %How "noisy" are the measurements.
    %0 is no noise, 0.2 is very noisy
end
...
options.numSteps = 1e6; %total iterations for MCMC loop.
options.mLPSCoefficient = 1e4; %max layers per step. 
%Set higher for longer 'burn-in' period
